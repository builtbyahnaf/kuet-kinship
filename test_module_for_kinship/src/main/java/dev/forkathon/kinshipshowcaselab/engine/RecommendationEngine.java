...
public class RecommendationEngine {
    private final Map<String, Double> recommendationScores = new LinkedHashMap<>();
    private final List<String> history = new ArrayList<>();
    private long revision = 0;

    public RecommendationEngine() { reset(); }
    ...
    public void record(String key, double score) {
        ...
        recommendationScores.put(normalized, Math.max(0.0, Math.min(1.0, score)));
    ...
    public double score(String key) {
        ...
        Double value = recommendationScores.get(key.trim().toLowerCase(Locale.ROOT));
        return value == null ? 0.0 : value;
    }

    public List<String> rankedKeys() {
        List<String> result = new ArrayList<>(recommendationScores.keySet());
        result.sort((a,b) -> Double.compare(recommendationScores.get(b), recommendationScores.get(a)));
        return result;
    }
    ...
    public Map<String, Double> snapshot() { return new LinkedHashMap<>(recommendationScores); }

    public String diagnostics() {
        return "RecommendationEngine: entries=" + recommendationScores.size() + ", history=" + history.size() + ", revision=" + revision;
    }
