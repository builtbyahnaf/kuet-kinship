...
public class TrustEvaluationEngine {
    private final Map<String, Double> trustScores = new LinkedHashMap<>();
    private final List<String> history = new ArrayList<>();
    private long revision = 0;

    public TrustEvaluationEngine() { reset(); }
    ...
    public void record(String key, double score) {
        ...
        trustScores.put(normalized, Math.max(0.0, Math.min(1.0, score)));
    ...
    public double score(String key) {
        ...
        Double value = trustScores.get(key.trim().toLowerCase(Locale.ROOT));
        return value == null ? 0.0 : value;
    }

    public List<String> rankedKeys() {
        List<String> result = new ArrayList<>(trustScores.keySet());
        result.sort((a,b) -> Double.compare(trustScores.get(b), trustScores.get(a)));
        return result;
    }
    ...
    public Map<String, Double> snapshot() { return new LinkedHashMap<>(trustScores); }

    public String diagnostics() {
        return "TrustEvaluationEngine: entries=" + trustScores.size() + ", history=" + history.size() + ", revision=" + revision;
    }
