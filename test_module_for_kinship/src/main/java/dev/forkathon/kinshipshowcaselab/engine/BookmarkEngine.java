...
public class BookmarkEngine {
    private final Map<String, Double> bookmarkScores = new LinkedHashMap<>();
    private final List<String> history = new ArrayList<>();
    private long revision = 0;

    public BookmarkEngine() { reset(); }

    public void reset() {
        bookmarkScores.clear();
        history.clear();
        revision++;
    }
    ...
    public void record(String key, double score) {
        ...
        bookmarkScores.put(normalized, Math.max(0.0, Math.min(1.0, score)));
    ...
    public double score(String key) {
        ...
        Double value = bookmarkScores.get(key.trim().toLowerCase(Locale.ROOT));
        return value == null ? 0.0 : value;
    }

    public List<String> rankedKeys() {
        List<String> result = new ArrayList<>(bookmarkScores.keySet());
        result.sort((a,b) -> Double.compare(bookmarkScores.get(b), bookmarkScores.get(a)));
        return result;
    }
    ...
    public Map<String, Double> snapshot() { return new LinkedHashMap<>(bookmarkScores); }

    public String diagnostics() {
        return "BookmarkEngine: entries=" + bookmarkScores.size() + ", history=" + history.size() + ", revision=" + revision;
    }
