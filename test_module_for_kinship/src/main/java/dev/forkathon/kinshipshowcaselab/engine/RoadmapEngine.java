...
public class RoadmapEngine {
    private final Map<String, Double> roadmapScores = new LinkedHashMap<>();
    private final List<String> history = new ArrayList<>();
    private long revision = 0;

    public RoadmapEngine() { reset(); }
    ...
    public void record(String key, double score) {
        ...
        roadmapScores.put(normalized, Math.max(0.0, Math.min(1.0, score)));
    ...
    public double score(String key) {
        ...
        Double value = roadmapScores.get(key.trim().toLowerCase(Locale.ROOT));
        return value == null ? 0.0 : value;
    }

    public List<String> rankedKeys() {
        List<String> result = new ArrayList<>(roadmapScores.keySet());
        result.sort((a,b) -> Double.compare(roadmapScores.get(b), roadmapScores.get(a)));
        return result;
    }
    ...
    public Map<String, Double> snapshot() { return new LinkedHashMap<>(roadmapScores); }

    public String diagnostics() {
        return "RoadmapEngine: entries=" + roadmapScores.size() + ", history=" + history.size() + ", revision=" + revision;
    }
