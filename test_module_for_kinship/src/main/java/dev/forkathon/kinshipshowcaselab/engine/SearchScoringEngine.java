...
public class SearchScoringEngine {
    private final Map<String, Double> searchScores = new LinkedHashMap<>();
    private final List<String> history = new ArrayList<>();
    private long revision = 0;

    public SearchScoringEngine() { reset(); }
    ...
    public void record(String key, double score) {
        ...
        searchScores.put(normalized, Math.max(0.0, Math.min(1.0, score)));
    ...
    public double score(String key) {
        ...
        Double value = searchScores.get(key.trim().toLowerCase(Locale.ROOT));
        return value == null ? 0.0 : value;
    }

    public List<String> rankedKeys() {
        List<String> result = new ArrayList<>(searchScores.keySet());
        result.sort((a,b) -> Double.compare(searchScores.get(b), searchScores.get(a)));
        return result;
    }
    ...
    public Map<String, Double> snapshot() { return new LinkedHashMap<>(searchScores); }

    public String diagnostics() {
        return "SearchScoringEngine: entries=" + searchScores.size() + ", history=" + history.size() + ", revision=" + revision;
    }
