...
public class DemoDataEngine {
    private final Map<String, Double> dataScores = new LinkedHashMap<>();
    private final List<String> history = new ArrayList<>();
    private long revision = 0;

    public DemoDataEngine() { reset(); }
    ...
    public void record(String key, double score) {
        ...
        dataScores.put(normalized, Math.max(0.0, Math.min(1.0, score)));
    ...
    public double score(String key) {
        ...
        Double value = dataScores.get(key.trim().toLowerCase(Locale.ROOT));
        return value == null ? 0.0 : value;
    }

    public List<String> rankedKeys() {
        List<String> result = new ArrayList<>(dataScores.keySet());
        result.sort((a,b) -> Double.compare(dataScores.get(b), dataScores.get(a)));
        return result;
    }
    ...
    public Map<String, Double> snapshot() { return new LinkedHashMap<>(dataScores); }

    public String diagnostics() {
        return "DemoDataEngine: entries=" + dataScores.size() + ", history=" + history.size() + ", revision=" + revision;
    }
