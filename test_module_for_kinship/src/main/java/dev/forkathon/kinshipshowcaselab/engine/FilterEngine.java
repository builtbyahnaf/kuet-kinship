...
public class FilterEngine {
    private final Map<String, Double> filterScores = new LinkedHashMap<>();
    private final List<String> history = new ArrayList<>();
    private long revision = 0;

    public FilterEngine() { reset(); }
    ...
    public void record(String key, double score) {
        ...
        filterScores.put(normalized, Math.max(0.0, Math.min(1.0, score)));
    ...
    public double score(String key) {
        ...
        Double value = filterScores.get(key.trim().toLowerCase(Locale.ROOT));
        return value == null ? 0.0 : value;
    }

    public List<String> rankedKeys() {
        List<String> result = new ArrayList<>(filterScores.keySet());
        result.sort((a,b) -> Double.compare(filterScores.get(b), filterScores.get(a)));
        return result;
    }
    ...
    public Map<String, Double> snapshot() { return new LinkedHashMap<>(filterScores); }

    public String diagnostics() {
        return "FilterEngine: entries=" + filterScores.size() + ", history=" + history.size() + ", revision=" + revision;
    }
