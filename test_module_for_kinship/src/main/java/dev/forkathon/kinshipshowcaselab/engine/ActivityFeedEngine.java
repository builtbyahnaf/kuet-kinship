...
public class ActivityFeedEngine {
    private final Map<String, Double> activityScores = new LinkedHashMap<>();
    private final List<String> history = new ArrayList<>();
    private long revision = 0;

    public ActivityFeedEngine() { reset(); }
    ...
    public void record(String key, double score) {
        ...
        activityScores.put(normalized, Math.max(0.0, Math.min(1.0, score)));
    ...
    public double score(String key) {
        ...
        Double value = activityScores.get(key.trim().toLowerCase(Locale.ROOT));
        return value == null ? 0.0 : value;
    }

    public List<String> rankedKeys() {
        List<String> result = new ArrayList<>(activityScores.keySet());
        result.sort((a,b) -> Double.compare(activityScores.get(b), activityScores.get(a)));
        return result;
    }
    ...
    public Map<String, Double> snapshot() { return new LinkedHashMap<>(activityScores); }

    public String diagnostics() {
        return "ActivityFeedEngine: entries=" + activityScores.size() + ", history=" + history.size() + ", revision=" + revision;
    }
