...
public class NotificationEngine {
    private final Map<String, Double> notificationScores = new LinkedHashMap<>();
    private final List<String> history = new ArrayList<>();
    private long revision = 0;

    public NotificationEngine() { reset(); }

    public void reset() {
        notificationScores.clear();
        history.clear();
        revision++;
    }
    ...
    public void record(String key, double score) {
        ...
        notificationScores.put(normalized, Math.max(0.0, Math.min(1.0, score)));
    ...
    public double score(String key) {
        ...
        Double value = notificationScores.get(key.trim().toLowerCase(Locale.ROOT));
        return value == null ? 0.0 : value;
    }

    public List<String> rankedKeys() {
        List<String> result = new ArrayList<>(notificationScores.keySet());
        result.sort((a,b) -> Double.compare(notificationScores.get(b), notificationScores.get(a)));
        return result;
    }
    ...
    public Map<String, Double> snapshot() { return new LinkedHashMap<>(notificationScores); }

    public String diagnostics() {
        return "NotificationEngine: entries=" + notificationScores.size() + ", history=" + history.size() + ", revision=" + revision;
    }
